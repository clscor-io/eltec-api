...
(: We create an initial config file the values of which can be passed by the
 ...
 :
 : - ELTEC_API_BASE: base URI the ELTeC API will be available under
 :)
  ...
  if(doc($config:file)/config) then
    ...
    local:store(
      ...
      <config>
        <api-base>
        {
          if (environment-variable("ELTEC_API_BASE")) then
            environment-variable("ELTEC_API_BASE")
          else "https://eltec.clscor.io/api"
        }
        </api-base>
      </config>
    )
