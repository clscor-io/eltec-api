...
ENV ELTEC_API_BASE=${ELTEC_API_BASE:-http://localhost:8080/exist/restxq}
...
COPY --from=builder /tmp/eltec-api/build/eltec-*.xar ${EXIST_HOME}/autodeploy/
...
RUN apt-get update \
    ...
    # set permissions for the eltec user
    && chown -R eltec:eltec ${EXIST_HOME} \
