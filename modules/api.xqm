...
module namespace api = "http://eltec.clscor.io/ns/exist/api";
...
function api:corpora-post-tei($data, $auth) {
  ...
  return if (not($header)) then
    ...
    let $corpus := eltei:get-corpus($name)
...
function api:corpus-texts($corpusname) {
  ...
  return
    if (not($corpus?name) or not(xmldb:collection-available($collection))) then
      ...
      array {eltei:get-corpus-text-info($corpusname)}
};
...
function api:text-info($corpusname, $textname) {
  let $info := eltei:get-text-info($corpusname, $textname)
  return
    if (count($info)) then
      $info
    else
  ...
  return
    ...
    then
      ...
      let $_ := (
        ...
        elutil:remove-sha($corpusname, $textname)
      )
      return $data
...
function api:text-plain($corpusname, $textname) {
  ...
  return
    if (count($doc)) then
      eltei:get-plain-text($corpusname, $textname)
    else
      <rest:response>
        <http:response status="404"/>
      </rest:response>
